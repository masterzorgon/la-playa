...
function VideoComponent({ isIntersecting }: VideoComponentProps) {
    return (
        <video
            preload="metadata"
            aria-label="Video player"
...
export function Hero() {
    ...
    return (
        <div className="relative overflow-hidden" ref={videoRef}>
            {isIntersecting ? (
                <VideoComponent isIntersecting={isIntersecting} />
            ) : (
                <FallBack />
            )}
